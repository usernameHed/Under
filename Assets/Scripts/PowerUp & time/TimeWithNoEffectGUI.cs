...
/// - utile pour les portals: pour qu'il y ai un temps minimal entre 2 téléportation.
/// Idem pour les powerUp ayant un intervalle entre 2 utilisation.
/// </summary>

public class TimeWithNoEffectGUI : MonoBehaviour
{
    ...
    private void Start()
    {
        timeToGo = Time.realtimeSinceStartup + timeOpti;
    }

    private void OnGUI()
    {
        if (Time.realtimeSinceStartup >= timeToGo)                                             //optimisation du CPU
        {
            ...
            {
                isOkIsFalse = true;
                timeStartWithNoEffect = Time.realtimeSinceStartup + (timeWithNoEffect * additionnalTime);
            }
            if (!isOk && isOkIsFalse && Time.realtimeSinceStartup >= timeStartWithNoEffect)    //la fin du timer
            {
                isOk = true;
