...
public class LevelManagerGame : MonoBehaviour
{
    ...
    public void activeLevelFailed()
    {
        if (displayMenuOnce)      //cette fonction est appelé qu'une fois !
            return;
        EventAnalytics.getSingularity().displayLevelRestart(PlayerPrefs.getSingularity().lastLevelPlayerId[0] + "-" + PlayerPrefs.getSingularity().lastLevelPlayerId[1]);
    ...
    public void HandleStatsVictorySolo()
    {
        ...
        //if (MapPrefs.PP.mapsInfosSolo[PlayerPrefs.getSingularity().lastLevelPlayerId].timeHighScore.IndexOf(gameManager.Chrono.GetComponents<ColoredProgressBar>()[0].currentProgress) < 0)
        //{
            MapPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]).timeHighScore.Add(gameManager.Chrono.GetComponents<ColoredProgressBar>()[0].currentProgress);
            MapPrefs.getSingularity().Save();
        //}
        EventAnalytics.getSingularity().OnLevelCompleted(true, PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1], gameManager.Chrono.GetComponents<ColoredProgressBar>()[0].currentProgress);
    }
    ...
    {
        if (PlayerPrefs.getSingularity().isLastMap(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]))    //si c'est la dernière map, change le bouton en "ending" (qui mène à une cinématique)
        {
        ...
        else if (PlayerPrefs.getSingularity().isLastLevelMap(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]))    //si c'est la dernière map d'un monde, change le bouton en "next World" (qui mène au menu)
        {
            listWindowButton[0].SetActive(false);
    ...
    void changeGoldenEggs()
    {
        ...
        // - 3 = gold
        // - 4 = epic
        for (int i = 0; i < MapPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]).timeEggs2to4.Count; i++)
        {
            if (currentScore <= MapPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]).timeEggs2to4[i])
                success++;
        }
        ...
        MI.succes = PlayerPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]).succes;//success;
        MI.bestHighScore = PlayerPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]).bestHighScore;// currentScore;
        MI.blocked = PlayerPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]).blocked; //map bloqué...

        //est-ce qu'on a gagné un nouvelle oeufs dans ce jeu ?
        if (success > PlayerPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]).succes)
        {
            newEggs = true;
            MI.succes = success;
        }
        ...
        if (currentScore < PlayerPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]).bestHighScore)
        {
            newScore = true;
        ...
        PlayerPrefs.getSingularity().setLevel(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1], MI);
        ...
        SoundManager.GetSingleton.MusicWin = true;
        ...
        if (nextType == 1)
        {
            ...
            if (MapPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1] + 1).isBonus)    //si la map suivante est une map bonus, check la bonus + la suivante !
            {
                if (success < MapPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1] + 2).succesNeededForUnloclk)
                {
                    //nextType
            ...
            else
            {
                ...
                if (success < MapPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1] + 1).succesNeededForUnloclk)
                {
                    //nextType
                    Debug.Log("ICI ON A PAS REUSSI");
                    nextType = 0;   //impossible de passer au level suivant !
        ...
        Debug.Log("worldunlock: " + PlayerPrefs.getSingularity().worldUnlock);
        Debug.Log("mapUnlockId: " + PlayerPrefs.getSingularity().mapUnlockId);
        Debug.Log("PlayerPrefs.getSingularity().lastLevelPlayerId[0]" + PlayerPrefs.getSingularity().lastLevelPlayerId[0]);
        Debug.Log("PlayerPrefs.getSingularity().lastLevelPlayerId[1]" + PlayerPrefs.getSingularity().lastLevelPlayerId[1]);
        Debug.Log("nextmapblocekd: " + PlayerPrefs.getSingularity().nextMapBlocked(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]));
        ...
        if (newEggs)
        {
            PlayerPrefs.getSingularity().setupCountGoldenEggs();
            MapPrefs.getSingularity().changeWorldUnlock();
        }

        PlayerPrefs.getSingularity().Save();
    }
    ...
    void unlockNextLevel(int isBonus = 0)
    {
        ...
        if (PlayerPrefs.getSingularity().worldUnlock == PlayerPrefs.getSingularity().lastLevelPlayerId[0]
            && PlayerPrefs.getSingularity().mapUnlockId == PlayerPrefs.getSingularity().lastLevelPlayerId[1] + isBonus
            && nextType != 0
            && PlayerPrefs.getSingularity().nextMapBlocked(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1] + isBonus))
        {
            Debug.Log("ICI SAVE QUE LA PROCHAINE MAP EST DEBLOQUE WOHOOO");
            PlayerPrefs.getSingularity().mapUnlockId += 1;                                              //débloque la suite des map solo !
            //TODO: si on a pas débloqué le bonus en meme temps, probleme ???

            PlayerPrefs.getSingularity().unlockTheNextLevel(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1] + isBonus);
        }
    }
    void unlockNextLevelBonus(int success)
    {
        int tmpMapUnlock = PlayerPrefs.getSingularity().mapUnlockId;
        bool alreadyJumped = true;
        //ici, d'abord tester si on a débloqué la map bonus + 1
        Debug.Log("ici nextLevel bonus: secces: " + success);
        //si la prochaine map du monde peut être débloqué
        if (PlayerPrefs.getSingularity().worldUnlock == PlayerPrefs.getSingularity().lastLevelPlayerId[0]
            && tmpMapUnlock == PlayerPrefs.getSingularity().lastLevelPlayerId[1]
            && nextType != 0
            && PlayerPrefs.getSingularity().nextMapBlocked(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1] + 1))
        {
            Debug.Log("ICI débloque la map après le bonus !");
            alreadyJumped = false;
            PlayerPrefs.getSingularity().mapUnlockId += 2;                                              //débloque 2 (fait un bon de 1)
            //TODO: si on a pas débloqué le bonus en meme temps, probleme ???

            PlayerPrefs.getSingularity().unlockTheNextLevel(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1] + 1);
        }

        //si on n'est pas passé par le déblocage de la map suivante, et qu'on l'a déja débloqué en faite...
        if (alreadyJumped
            && !PlayerPrefs.getSingularity().nextMapBlocked(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1] + 1)
            && PlayerPrefs.getSingularity().nextMapBlocked(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1])
            && nextType != 0
            )
        {
            ...
            if (success >= MapPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1] + 1).succesNeededForUnloclk)
            {
                ...
                PlayerPrefs.getSingularity().unlockTheNextLevel(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]);
            }
        }
        ...
        else if (PlayerPrefs.getSingularity().worldUnlock == PlayerPrefs.getSingularity().lastLevelPlayerId[0]
            && tmpMapUnlock == PlayerPrefs.getSingularity().lastLevelPlayerId[1]
            && nextType != 0
            && PlayerPrefs.getSingularity().nextMapBlocked(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]))
        {
    ...
    public void jumpNextScene()
    {
        Time.timeScale = 1;
        FPS.enabled = true;
        if (PlayerPrefs.getSingularity().isLastMap(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]))                                        //ending
        {
    ...
    void handleCorectQuitForMainMenu()
    {
        if (!GVM.multi)
        {
            ...
            if (PlayerPrefs.getSingularity().lastLevelPlayerId[0] == 0)
    ...
    public void restartScene()
    {
        ...
        if (quitIfFromUnity())
            return;
        GVM.fromGame = false;
        PlayerPrefs.getSingularity().fromRestart = true;
        if (!GVM.multi)
            QOC.jumpToSceneWithFade(PlayerPrefs.getSingularity().jumpToSpecificLevel(PlayerPrefs.getSingularity().lastLevelPlayerId[0], PlayerPrefs.getSingularity().lastLevelPlayerId[1]));
        else
            QOC.jumpToSceneWithFade(MapPrefs.getSingularity().mapsInfosMulti[PlayerPrefs.getSingularity().mapUnlockMulti].nameLevel);
    }
