...
public class LevelManagerMulti : MonoBehaviour
{
    ...
    public void justPlay()
    {
        fillPlayerList();
        Global.displayValue();
        //PlayerPrefs.getSingularity().lastLevelPlayerId[1] = MapPrefs.PP.mapsInfosMulti[mapLocation].nameLevel;
        PlayerPrefs.getSingularity().lastLevelPlayerId[1] = MapPrefs.getSingularity().mapsInfosMulti[mapLocation].mapId;
        //if (mapLocation < MD.dms.Count && MD.dms[mapLocation].worldId == MD.dms[mapLocation + 1].worldId)
        //  PlayerPrefs.getSingularity().nextLevel = MD.dms[mapLocation + 1].nameLevel;
        PlayerPrefs.getSingularity().Save();
        QOC.jumpToScene(MapPrefs.getSingularity().mapsInfosMulti[mapLocation].nameLevel);
