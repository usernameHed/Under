...
namespace Rewired.Demos {
    ...
    public class ControlRemapping : MonoBehaviour {
        ...
        private void Awake() {
            ...
            PC = gameObject.GetComponent<PlayerConnected>();
            Initialize();
        }
        ...
        void initController()
        {
            foreach (Player player in ReInput.players.GetPlayers(true))
            {
                ...
                {
                    PC.updatePlayerController(player.id, true);
                    break;
                }
            }
        }
        ...
        /// <summary>
        /// lorsqu'on ferme le dialog, pas interessants
        /// </summary>
        /// <param name="queueActionId"></param>
            ...
            if(ReInput.controllers.IsControllerAssigned(args.controllerType, args.controllerId))
            {
                ...
                foreach(Player player in ReInput.players.AllPlayers)
                {
                    if(player.controllers.ContainsController(args.controllerType, args.controllerId))
                    {
                        ...
                        Debug.Log("ici player " + player.id + " se connecte !");
                        PC.updatePlayerController(player.id, true);
                    }
                }
            }
            ...
            if(showMenu)
            {
                if(ReInput.controllers.IsControllerAssigned(args.controllerType, args.controllerId))
                {
                    foreach(Player player in ReInput.players.AllPlayers)
                    {
                        ...
                        Debug.Log("ici player " + player.id + " se déconnecte !");
                        PC.updatePlayerController(player.id, false);
                    }
                }
                else
                {
