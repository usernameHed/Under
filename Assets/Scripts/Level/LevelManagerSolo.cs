...
public class LevelManagerSolo : MonoBehaviour
{
    ...
    void HandleNextLevel()
    {
        if (Global.fromGame)                        //si on viens du game (nextLevel)
        {
            ...
            mapLocation = PlayerPrefs.getSingularity().lastLevelPlayerId[1] + 1;             //set la nouvelle mapLocation (pour le justPlay) à l'ancienne + 1 (la prochaine map !)
            ...
            if (!(Global.soloAnd2Player && powerMapAccepted.Count > 1 && MapPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], mapLocation).coop))
                Global.soloAnd2Player = false;
            else
            {
    ...
    public void changeIndexMap()
    {
        ...
        if (joypadConnected >= 2 && powerMapAccepted.Count > 1 && MapPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], indexMap).coop)       //si il y a présente de 2+ manette
        {
    ...
    void setIfNextMapIsBlocked()
    {
        ...
        {
            PlayerPrefs.getSingularity().mapNextBlocked = true;
        }
        else
            PlayerPrefs.getSingularity().mapNextBlocked = false;*/
    }
    ...
    public void justPlay()
    {
        ...
        PlayerPrefs.getSingularity().lastLevelPlayerId = mapLocation;                             //set l'id de la map qui va jouer
        PlayerPrefs.getSingularity().lastLevelPlayed = MapPrefs.PP.mapsInfosSolo[mapLocation].nameLevel;             //set le nom de la map qui va jouer

        //si on autorise le next level, OK, sinon, nop
        if (testIfNextMapIsPossible())
            PlayerPrefs.getSingularity().nextLevel = MapPrefs.PP.mapsInfosSolo[mapLocation + 1].nameLevel;
        else
            PlayerPrefs.getSingularity().nextLevel = "None";
    ...
    private void playerInput()
    {
        ...
        if (mainLocation == 1 && TWNE.isOk)                                             //on est dans le choix du mode
        {
            ...
            //joypad 1, ou clavier ZQSD ou clavier arrow UP
                        ...
                        && (powerMapAccepted.Count > 1 && MapPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], mapLocation).coop))
   //TODO ???
            {
                locationCoop = 1;
                panelSoloOrCoop[0].isOn = false;
        ...
        if (mainLocation != prevMainLocation)                       //change si il a changé
        {
            ...
            switch (mainLocation)
            {
                case 0:                                             //si les joueurs sont dans la sélection de map
                    ...
                    break;
                case 1:
                    if (powerMapAccepted.Count <= 1 || !MapPrefs.getSingularity().getLevels(PlayerPrefs.getSingularity().lastLevelPlayerId[0], mapLocation).coop)                    //si la map n'autorise qu'une seul pouvoir...
                    {
