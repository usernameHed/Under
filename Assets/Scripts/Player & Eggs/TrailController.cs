    ...
    public void powerFlare(int state)
    {
        switch (state)
        {
            case 0:                         //press pour la premiere fois
                ...
                SoundManager.GetSingleton.playSound("CreateTrail" + PC.nb_player);
                //SoundManager.getSingularity().playSound("Green Power Start", PC.nb_player, CPB.getProgress() * 1.0f / 1000.0f);  //commence le son avec le start courrant de vitesse
                createTrail();
                break;
            ...
            case 2:                         //relache !
                ...
                SoundManager.GetSingleton.playSound("ReleaseTrail" + PC.nb_player, true);
                //SoundManager.getSingularity().playSound("Green Power End", PC.nb_player, CPB.getProgress() * 1.0f / 1000.0f);
                releaseTrail();
                break;
            case 3:
                PC.speed = PC.oldSpeed;
                SoundManager.GetSingleton.playSound("KillTrail" + PC.nb_player);
                //SoundManager.getSingularity().playSound("Green Power Stop", PC.nb_player, CPB.getProgress() * 1.0f / 1000.0f);
                clearAll();                 //kill all !
                break;
        }
    }
